...
What kinds of versioning make sense in a CICD world? What makes sense to use with Kubernetes? How do we ensure each container image is truely unique, so Kubernetes will represent our correct state?
...
This has served us well, it has some very distict advantages over some of the later methods. 
...
- Differentaiting for specific rollout strategies like canary or blue/green style deployments
...
- Kubernetes doesn't differentiate between any container updates without a specific container to hook onto.
  - i.e. `1.0.1` pushed at 2pm is the same conatiner as `1.0.1` at 5pm. Its not something we should be doing, but we all know it happens. 
- It becomes a problem of combining lots of little updates into a peticular patch version. 
...
For many of us version control is synonimous with git. For good reason. 

Each commit we push to a repository will generate a git hash. It's a 160 big SHA-1 string calculated from your repo contents. If you change so much as a `!` anywhere in the commited code you will get a unique identifier. I'm sure I'll get comments saying its not truely unique, it just has a really high chance of avoiding collisions. So it is **effectively** unique. So we'll leave it there.
...
You get a couple of downsites, the most major one would be that it is difficult for humans to know the order of releases. If you are utilizing a full CICD pipeline this doesn't matter as much because everything we're doing is programatic. 

We can pinpoint exactly when something went wrong but looking at our merge history, if a group commits several times a day, semantic versioning sometimes isn't the best choice. Its generally a great idea to use these in tandum, with specific organizational reasons pertaining to each version update, even if they are a `patch` update. 

Codefresh will automatically put this value into the pipeline environment variable for us to use. this is references with `CF_Revision` or even `CF_SHORT_REVISION`. The second one is sort of self explanitory, it will only produce the first 7 characters in the hash.
...
Build ID can be very useful as well. Instead of our unique ID being tied to the state of our repository or SemVer its tied directly to the pipeline. When trying to pinpoint errors with our build process or when facing a monorepo it can be great to trace issues for each piece of the puzzle, it helps find issues early and often if used correctly.
...
Since Codefresh uses a tags array you can append as many, or as few versions to upload to git hub. You can seperate out builds into multi part builds. 
